...
void http_task::run()
{
    ...
    socketfd->delete_ptr_hook = [](void *ptr) -> void
    {
        if (ptr != nullptr)
        {
            session::http_session *m_http_session = (session::http_session *)ptr;
            delete m_http_session;
        }
    };
    if (socketfd->ptr == nullptr)
    {
        session::http_session *m_http_session = new session::http_session(socketfd->get_fd());
        socketfd->ptr = (void *)m_http_session;
    }
    session::http_session *m_http_session = (session::http_session *)socketfd->ptr;
    ...
    {
        session::http_session *m_session = static_cast<session::http_session *>(parser->data);
        m_session->add_to_body(at, length);
        return 0;
    };
    ...
    while (1)
    {
        len = socketfd->recv(m_http_session->buffer, m_http_session->buffer_size);
        ...
        else if (len == 0) // noting recv later
        {
            ...
            m_http_session->set_over(true);
            break;
        }
        else if (len > 0)
        {
            int nparsed = http_parser_execute(m_http_session->get_parser(), &settings, m_http_session->buffer, len);
            if (m_http_session->get_parser()->upgrade)
            {
            }
...
void http_task::test(const session::http_session *m_session)
{
